...
async def main() -> NoReturn:
    """
    Функция отправляет запросы к API isDayOff() 
    для получения статусов следующих трёх дней.

    Приходится делать 3 запроса для каждого дня, так как API 
    пока не поддерживает идентификацию праздничных дней при 
    получении ответа на запрос типа дата1-дата2.

    Ниже есть закоментированный пример с 9 мая.
    С помощью него можно убедиться в том, 
    что скрипт работает и создаёт задачу в Битрикс24, 
    если в ближайшие 3 дня будет государственный праздник.
    """
    # Пример с 9 мая:
